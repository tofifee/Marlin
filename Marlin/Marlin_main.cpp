...
inline void gcode_M42() {
  ...
  inline void gcode_M48() {
    ...
    double mean = 0, sigma = 0, sample_set[n_samples], zmin = 99999, zmax = 0;
    ...
    for (uint8_t n = 0; n < n_samples; n++) {
      ...
      if(sample_set[n] < zmin) zmin = sample_set[n];
      if(sample_set[n] > zmax) zmax = sample_set[n];
      ...
      if (verbose_level > 0) {
        if (verbose_level > 1) {
          ...
          if (verbose_level > 2) {
            ...
            SERIAL_PROTOCOL_F(zmin, 3);
            SERIAL_PROTOCOLPGM(" max: ");
            SERIAL_PROTOCOL_F(zmax, 3);
            SERIAL_PROTOCOLPGM(" range: ");
            SERIAL_PROTOCOL_F(zmax-zmin, 3);
          }
        }
        SERIAL_EOL;
      }
